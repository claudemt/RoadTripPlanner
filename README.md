...
网站模式下，路线按登录用户隔离保存。景点介绍属于共享资料，登录用户可以共同补充和维护。
...
`data/routes/` 和 `data/config/local.env` 属于个人数据，不提交到 Git。
...
网站模式下，景点资料保存在 Supabase 中，由登录用户共同维护。
...
| `data/scenes/` | 本地共享景点资料 |
| `data/routes/` | 本地私有路线和导出文件 |
| `data/config/` | 本地私有配置 |
| `docs/` | 项目文档 |
