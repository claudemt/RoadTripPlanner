...
一个同时支持正式网站和本地高级导出的自驾路线规划工具。网站版使用静态前端，Casdoor 负责注册登录，Supabase 负责数据库、用户路线隔离、共享景点和图片存储；本地版继续提供 Markdown/PDF 和 Remotion MP4 导出。
...
5. 按 `app/.env.example` 添加 Supabase 与 Casdoor provider 环境变量；高德 Key 由 admin 登录网站后在配置页维护。
