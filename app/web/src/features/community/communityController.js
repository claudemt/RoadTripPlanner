(function () {
  function create({
    ...
    function renderContributionBlocks(result, prefix = 'profile') {
      ...
      if (summary) {
        summary.textContent = `贡献 ${contributions.routeCount || 0} 条公共路线，${contributions.sceneCount || 0} 个景点介绍`;
      }
    }
    ...
    function messageMarkup(item) {
      ...
      const actions = item.withdrawn ? '' : `
        <div class="community-message-actions">
          ...
          ${!item.mine && runtime.isAdmin && channel === 'public' ? `<button type="button" data-withdraw-id="${escapeAttr(item.id)}">删除</button>` : ''}
        </div>
      `;
      return `
    ...
    async function refreshMessages({quiet = false} = {}) {
      ...
      const result = channel === 'direct' && directPeer
        ? await localService.listDirectMessages(directPeer.email, 120)
        : await localService.listCommunityMessages(120);
    ...
    function renderChannelState() {
      const publicButton = el('communityPublicBtn');
      const directButton = el('communityDirectBtn');
      const heading = el('communityThreadHead');
      publicButton?.classList.toggle('active', channel === 'public');
      directButton?.classList.toggle('active', channel === 'direct');
      if (directButton) {
        directButton.hidden = !directPeer;
        directButton.textContent = directPeer ? `与 ${directPeer.nickname || directPeer.email} 私聊` : '私聊';
      }
      if (heading) {
        heading.textContent = '';
        if (channel === 'direct' && directPeer) {
          const profileButton = document.createElement('button');
          profileButton.type = 'button';
          profileButton.textContent = `与 ${directPeer.nickname || directPeer.email} 的私聊`;
          profileButton.onclick = () => openProfile(directPeer.email).catch((error) => toast(error.message));
          heading.appendChild(profileButton);
        }
      }
      const input = el('communityMessageInput');
      if (input) {
        input.placeholder = channel === 'direct' && directPeer
          ? `发送给 ${directPeer.nickname || directPeer.email} 的消息`
          : '发送消息';
      }
    }

    async function setChannel(nextChannel) {
      if (nextChannel === 'direct' && !directPeer) return;
      channel = nextChannel === 'direct' ? 'direct' : 'public';
      replyTarget = null;
      setReply(null);
      renderChannelState();
    ...
    async function startDirectChat(profile) {
      if (!profile?.email || profile.email === selfProfile?.email) return;
      directPeer = profile;
      channel = 'direct';
      renderChannelState();
      dialogs.close('profileModal');
      openAccountCenter?.();
      setAccountView?.('community');
      await refreshMessages();
    }
    ...
    async function sendMessage() {
      ...
      const {response, data} = await localService.postCommunityMessage({
        ...
        recipientEmail: channel === 'direct' ? directPeer?.email : null,
      });
    ...
    async function withdrawMessage(messageId) {
      ...
      const isOwn = Boolean(target?.mine);
      if (!confirm(isOwn ? '撤回这条消息？' : '删除这条消息？')) return;
      const {response, data} = await localService.withdrawCommunityMessage(messageId);
      if (!response.ok || !data?.ok) throw new Error(data?.message || (isOwn ? '撤回失败' : '删除失败'));
      await refreshMessages({quiet: true});
    }
    ...
    function profileBody(result) {
      ...
      return `
        ...
        <div class="profile-modal-stats">
          ...
          <div><strong>${escapeHtml(contributions.sceneCount || 0)}</strong><span>景点介绍</span></div>
        </div>
        ${profile.email !== selfProfile?.email ? `<button class="primary profile-start-direct" type="button" data-start-direct-email="${escapeAttr(profile.email)}">发起私聊</button>` : ''}
      `;
    }
    ...
    function bind() {
      ...
      el('communityPublicBtn').onclick = () => setChannel('public').catch((error) => toast('切换公聊失败：' + error.message));
      el('communityDirectBtn').onclick = () => setChannel('direct').catch((error) => toast('读取私聊失败：' + error.message));
      ...
      el('profileModalBody').onclick = (event) => {
        const direct = event.target.closest('[data-start-direct-email]');
        if (!direct) return;
        localService.getProfile(direct.dataset.startDirectEmail)
          .then(({response, data}) => {
            if (!response.ok || !data?.ok) throw new Error(data?.message || '无法读取用户资料');
            return startDirectChat(data.profile);
          })
          .catch((error) => toast('打开私聊失败：' + error.message));
      };
    }
