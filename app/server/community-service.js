...
function createCommunityService({
  ...
  const getContributions = async (email) => {
    ...
    const sceneNames = new Set((scenes.data || []).map((item) => item.normalized_name).filter(Boolean));
    return {
  ...
  const getProfileSummaries = async (emails) => {
    ...
    if (error) throw error;
    const rows = new Map((data || []).map((item) => [item.owner_email, item]));
    const profiles = await Promise.all(normalizedEmails.map(async (email) => [
      email,
      await serializeProfile(email, rows.get(email) || null),
    ]));
  ...
  const listMessages = async ({limit = 100} = {}, identity) => {
    ...
    const {data, error} = await getSupabase()
      ...
      .select('id,author_email,body,attachments,reply_to_id,channel,recipient_email,conversation_key,withdrawn_at,created_at,updated_at')
      .eq('channel', 'public')
      .eq('conversation_key', 'public')
      .order('created_at', {ascending: false})
      .limit(safeLimit);
    if (error) throw error;
    return serializeMessages((data || []).reverse(), identity, 'public');
  };

  const listDirectMessages = async ({email: requestedEmail, limit = 100} = {}, identity) => {
    const ownerEmail = normalizeEmail(identity?.email);
    const peerEmail = normalizeEmail(requestedEmail);
    if (!ownerEmail || !peerEmail) throw new Error('私聊对象无效。');
    if (ownerEmail === peerEmail) throw new Error('不能和自己私聊。');
    const conversationKey = [ownerEmail, peerEmail].sort().join('|');
    const safeLimit = Math.max(1, Math.min(200, Number(limit) || 100));
    const {data, error} = await getSupabase()
      .from('roadtrip_forum_messages')
      .select('id,author_email,body,attachments,reply_to_id,channel,recipient_email,conversation_key,withdrawn_at,created_at,updated_at')
    ...
    return serializeMessages(messages, identity, 'direct');
  };

  const serializeMessages = async (messages, identity, channel) => {
    const supabase = getSupabase();
    ...
    const profiles = await getProfileSummaries(emails);
    return {
      ok: true,
      messages: await Promise.all(messages.map(async (item) => {
        ...
        return {
          id: item.id,
          author: profiles[item.author_email] || await serializeProfile(item.author_email),
          body: withdrawn ? '' : item.body,
          attachments: withdrawn ? [] : await resolveAttachments(item.attachments),
          replyTo: reply ? {
            id: reply.id,
            author: profiles[reply.author_email] || await serializeProfile(reply.author_email),
  ...
  const postMessage = async (payload, identity) => {
    ...
    const channel = recipientEmail ? 'direct' : 'public';
    if (recipientEmail && recipientEmail === email) throw new Error('不能和自己私聊。');
    const conversationKey = recipientEmail ? [email, recipientEmail].sort().join('|') : 'public';
    const replyToId = String(payload?.replyToId || '').trim() || null;
    if (replyToId) {
      const {data: reply, error: replyError} = await supabase
        .from('roadtrip_forum_messages')
        .select('id,channel,conversation_key')
        .eq('id', replyToId)
        .maybeSingle();
      if (replyError) throw replyError;
      if (!reply) throw new Error('引用的消息不存在。');
      if (reply.channel !== channel || reply.conversation_key !== conversationKey) {
        throw new Error('只能引用当前会话中的消息。');
      }
    }
  ...
  const withdrawMessage = async (messageId, identity) => {
    ...
    const {data: existing, error: readError} = await supabase
      .from('roadtrip_forum_messages')
      .select('id,author_email,attachments,withdrawn_at,channel')
      .eq('id', id)
      .maybeSingle();
    if (readError) throw readError;
    if (!existing) {
    ...
    const canModerate = isAdmin(identity) && existing.channel === 'public';
    if (existing.author_email !== email && !canModerate) {
